...
import type { Connection, PinElement, PinType, Node } from '.'
...
export type EdgeUpdatable = boolean | PinType
...
export type DefaultEdgeOptions = Omit<
  Edge,
  'id' | 'output' | 'input' | 'outputPin' | 'inputPin' | 'outputNode' | 'inputNode'
>

export type EdgeMouseHandler = (event: ReactMouseEvent, edge: Edge) => void

export type WrapEdgeProps<T = any> = Omit<Edge<T>, 'outputPin' | 'inputPin'> & {
  ...
  onEdgeUpdateStart?: (event: ReactMouseEvent, edge: Edge, pinType: PinType) => void
  onEdgeUpdateEnd?: (event: MouseEvent | TouchEvent, edge: Edge, pinType: PinType) => void
  rfId?: string
  isFocusable: boolean
...
export type EdgeProps<T = any> = Pick<
  Edge<T>,
  'id' | 'animated' | 'data' | 'style' | 'selected' | 'output' | 'input'
> &
  Pick<
    WrapEdgeProps,
    | 'outputX'
